...
class JUnitTestRefFactoryImpl @Inject constructor(
    ...
    object JUnitNoOpTestRef : JUnitTestRef {
        ...
        override operator fun get(testResults: Map<TestIdentifier, TestRef.Result>): TestRef.Result =
            TestRef.Result.aborted(NoOpFailedError())
    }
    ...
    class Default(private val testSource: TestSource) : JUnitTestRef {
        ...
        override operator fun get(testResults: Map<TestIdentifier, TestRef.Result>): TestRef.Result {
            ...
            return TestRef.Result.failed(TestNotFoundError())
        }
    }
    ...
    class Not(private val testRef: JUnitTestRef) : JUnitTestRef {
        ...
        override operator fun get(testResults: Map<TestIdentifier, TestRef.Result>): TestRef.Result {
            return if (testRef[testResults].status == TestRef.Result.Status.SUCCESSFUL) {
                TestRef.Result.failed(NotFailedError())
            } else {
                TestRef.Result.successful()
            }
        }
    }
    ...
    companion object {
        fun Array<out JUnitTestRef>.execute(
            testResults: Map<TestIdentifier, TestRef.Result>,
            exceptionSupplier: (String) -> Throwable,
            predicate: (List<TestRef.Result>) -> Boolean,
        ): TestRef.Result {
            val notSuccessful = asSequence()
                .map { it[testResults] }
                .filter { it.status != TestRef.Result.Status.SUCCESSFUL }
                .toList()
            return if (predicate(notSuccessful)) {
                notSuccessful.asSequence()
                    ...
                    .let(TestRef.Result::failed)
            } else TestRef.Result.successful()
        }
    }
}
