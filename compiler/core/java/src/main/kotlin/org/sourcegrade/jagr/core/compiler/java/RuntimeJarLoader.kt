...
import org.sourcegrade.jagr.agent.compiler.ResourceCollectorImpl
import org.sourcegrade.jagr.agent.compiler.ResourceExtractor
import org.sourcegrade.jagr.core.compiler.RuntimeResourceContainer
...
class RuntimeJarLoader @Inject constructor(
    ...
    fun loadCompiled(
        ...
        val classStorage: MutableMap<String, JavaCompiledClass> = mutableMapOf()
        ...
        for (resource in container) {
            when {
                resource.name.endsWith(".class") -> {
                    ...
                    classStorage[className] = JavaCompiledClass.Existing(className, resource.getInputStream().use { it.readBytes() })
                }
        ...
        return JavaRuntimeResourceContainer(container.info, resourceCollector, RuntimeResources(classStorage, resources))
    }
    ...
    fun compileSources(
    ...
    ): JavaCompiledResourceContainer {
        if (source.sourceFiles.isEmpty()) {
            // no source files, skip compilation task
            return JavaCompiledResourceContainer(source, messages = source.messages)
        }
        val compiledClasses: MutableMap<String, JavaCompiledClass> = mutableMapOf()
        ...
        if (!result || collector.diagnostics.isNotEmpty()) {
            ...
            return JavaCompiledResourceContainer(
                source,
                newRuntimeResources,
                messages,
                warnings,
        ...
        return JavaCompiledResourceContainer(source, newRuntimeResources)
    }
}
